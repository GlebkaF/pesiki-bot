...
for(const file of (await readdir(dir)).filter(f=>/^\d+\.dem\.archive$/.test(f)).sort().reverse()){
 ...
 if(old?.parser_version===(process.env.TARGET_PARSER_VERSION||"pesiki-replay-v9")&&old.analytics_version===(process.env.TARGET_ANALYTICS_VERSION||"combat-log-v3")&&(old.analytics_version!=="combat-log-v3"||old.combat_timeline?.version==="combat-timeline-v1")&&old.players.every(p=>p.combat_details?.version===old.analytics_version&&(old.parser_version!=="pesiki-replay-v9"||p.combat_details?.coverage?.ward_destroy_semantics===true))){report.skipped++;continue;}
 ...
 try{
  ...
  if(parsed.parser_version!==(process.env.TARGET_PARSER_VERSION||"pesiki-replay-v9")||parsed.analytics_version!==(process.env.TARGET_ANALYTICS_VERSION||"combat-log-v3")||
    (parsed.analytics_version==="combat-log-v3"&&parsed.combat_timeline?.version!=="combat-timeline-v1")||
    (parsed.parser_version==="pesiki-replay-v9"&&!parsed.players.every(p=>p.combat_details?.coverage?.ward_destroy_semantics===true)))throw Error("Parser output does not match requested analytics version/timeline/ward semantics");
