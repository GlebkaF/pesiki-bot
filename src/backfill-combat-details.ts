...
for(const file of (await readdir(dir)).filter(f=>/^\d+\.dem\.archive$/.test(f)).sort().reverse()){
 ...
 if(old?.parser_version===(process.env.TARGET_PARSER_VERSION||"pesiki-replay-v7")&&old.analytics_version===(process.env.TARGET_ANALYTICS_VERSION||"combat-log-v3")&&(old.analytics_version!=="combat-log-v3"||old.combat_timeline?.version==="combat-timeline-v1")&&old.players.every(p=>p.combat_details?.version===old.analytics_version)){report.skipped++;continue;}
 ...
 try{
  ...
  if(parsed.parser_version!==(process.env.TARGET_PARSER_VERSION||"pesiki-replay-v7")||parsed.analytics_version!==(process.env.TARGET_ANALYTICS_VERSION||"combat-log-v3")||
