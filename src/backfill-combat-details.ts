...
for(const file of (await readdir(dir)).filter(f=>/^\d+\.dem\.archive$/.test(f)).sort().reverse()){
 ...
 if(old?.parser_version===(process.env.TARGET_PARSER_VERSION||"pesiki-replay-v6")&&old.analytics_version===(process.env.TARGET_ANALYTICS_VERSION||"combat-log-v2")&&old.players.every(p=>p.combat_details?.version===old.analytics_version)){report.skipped++;continue;}
