...
export class ApmStore {
  ...
  saveReplay(match: ParsedMatch): boolean {
    ...
    if(match.hero_death_journal!==undefined)assertHeroDeathJournal(match.hero_death_journal);
    ...
    const stored = {...match, combat_timeline, hero_death_journal:match.hero_death_journal??existing?.hero_death_journal, ward_lifetimes:match.ward_lifetimes??existing?.ward_lifetimes, ward_events:match.ward_events??existing?.ward_events, analytics_version:match.analytics_version??existing?.analytics_version, start_time: match.start_time ?? existing?.start_time, players: match.players.map(p => {
  ...
  mergeReplayAnalytics(parsed: ParsedMatch): void {
    ...
    this.db.transaction(()=>{
      ...
      this.saveReplay({...latest,analytics_version:parsed.analytics_version,parser_version:parsed.parser_version,
        hero_death_journal:parsed.hero_death_journal??latest.hero_death_journal,
