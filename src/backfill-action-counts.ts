...
const dir=process.env.REPLAY_ARCHIVE_DIR||path.join(process.env.DATA_DIR||"data","replay-archives");
...
for(const file of (await readdir(dir)).filter(f=>/^\d+\.dem\.archive$/.test(f)).sort()) {
  ...
  if(old?.players.length&&old.players.every(p=>p.actions!==undefined&&validActionCounts(p.action_counts,p.actions))) {skipped++;continue;}
  ...
  try {
    ...
    assertReplayIdentity(parsed,id,store.matchApi(id));
    if(!parsed.apm_version||!parsed.players.every(p=>p.actions!==undefined&&validActionCounts(p.action_counts,p.actions))) throw Error("Missing or invalid action breakdown");
